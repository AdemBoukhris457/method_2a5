...
if (empty($_SESSION['e'])) {
    ...
    $listeAliments = $pC->afficherpagin($start_from, $num_per_page, $_SESSION['lista']);
    if (isset($_POST['search'])) {
        $valueToSearch = $_POST['valueToSearch'];
        $listeAliments = $AlimentC->afficherrech($valueToSearch);
    } else {
        $listeAliments = $pC->afficherpagin1($start_from, $num_per_page, $_SESSION['lista']);
    }
    $numa = 05;
    $star = ($page - 1) * 05;
    if (isset($_POST['ASC'])) {
...
<body>
                        ...
                        <div class="probootstrap-block-image">
                            ...
                            <div class="text">
                                <td><span class="date"><?PHP echo $user['nb_calories']; ?> calories</span>
                                    ...
                                    <a href="afficherPseul.php?id_produit=<?PHP echo $user['id_produit']; ?>"> details </a>
    ...
    <?php
    $liste = $AlimentC->afficherProduitsUtilisateur($_SESSION['lista']);
    $total_record = $liste->rowCount();
    $total_page = ceil($total_record / $num_per_page);
    if ($page > 1) {
        echo "<a href='affichernew1.php?page=" . ($page - 1) . "' class='btn btn-danger'>Previous</a>";
    }


    for ($i = 1; $i < $total_page + 3; $i++) {
        echo "<a href='affichernew1.php?page=" . $i . "' class='btn btn-primary'>$i</a>";
    }

    if ($i > $page) {
        echo "<a href='affichernew1.php?page=" . ($page + 1) . "' class='btn btn-danger'>Next</a>";
    }
    ?>
</body>
